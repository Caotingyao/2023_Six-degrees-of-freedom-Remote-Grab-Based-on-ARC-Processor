...
void soft_i2c_gpio_init(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5 | GPIO_Pin_7; // PA5/PA7=外接I2C
 ...
 * 输    出         : 0: 成功 1：失败
 *******************************************************************************/
uint8_t i2c_mem_write(uint8_t DevAddress, uint8_t MemAddress, uint16_t Len, uint8_t *pData)
{
    i2c_start();
 ...
 * 输    出         : 0: 成功 1：失败
 *******************************************************************************/
uint8_t i2c_mem_read(uint8_t DevAddress, uint8_t MemAddress, uint16_t Len, uint8_t *pBuffer)
{
    i2c_start();
...
{
    uint8_t b;
    if (!i2c_mem_read(DevAddress, addr, 1, &b))
    {
        b = (Data != 0) ? (b | (1 << bitNum)) : (b & ~(1 << bitNum));
        return i2c_mem_write(DevAddress, addr, 1, &b); // 写入数据
    }
    else
        return 1;
}
